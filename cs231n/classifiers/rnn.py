...
class CaptioningRNN(object):
    ...
    def loss(self, features, captions):
        ...
        if self.cell_type == 'rnn':
          ...
          loss, grad1 = temporal_softmax_loss(out4, captions_out, mask)
          grad2, grads["W_vocab"], grads["b_vocab"] = temporal_affine_backward(grad1, cache4)
          grad3, grad4, grads["Wx"], grads["Wh"], grads["b"] = rnn_backward(grad2, cache3)
          grads["W_embed"] =  word_embedding_backward(grad3, cache2)
          grad5, grads["W_proj"], grads["b_proj"] = affine_backward(grad4, cache1)
        elif self.cell_type == 'lstm':
          out3, cache3 = lstm_forward(out2, out1, Wx, Wh, b)
          out4, cache4 = temporal_affine_forward(out3, W_vocab, b_vocab)
          loss, grad1 = temporal_softmax_loss(out4, captions_out, mask)
          grad2, grads["W_vocab"], grads["b_vocab"] = temporal_affine_backward(grad1, cache4)
          grad3, grad4, grads["Wx"], grads["Wh"], grads["b"] = lstm_backward(grad2, cache3)
          grads["W_embed"] =  word_embedding_backward(grad3, cache2)
          grad5, grads["W_proj"], grads["b_proj"] = affine_backward(grad4, cache1)
        else:
          raise ValueError("cell type should be 'rnn' or 'lstm'!")
